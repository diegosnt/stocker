...
32. **Peso por Activo + Ganancias y Pérdidas por Activo** (Análisis) — dos gráficos nuevos en la sección de tenencia actual. El primero, "Peso por Activo en la Cartera", en realidad **ya existía como código**: `_renderAssetColumnChart` seguía llamándose en cada render, pero el contenedor que necesitaba (`#current-holdings-chart` / `#asset-distribution-card`) había desaparecido del HTML en algún cambio anterior, así que la función hacía early-return sin dibujar nada — se restauró el contenedor y volvió a funcionar. El segundo, "Ganancias y Pérdidas por Activo", es nuevo: barras horizontales verde/rojo (`ChartManager.renderBarChart`, existía en `chart-manager.js` pero no se usaba en esta página) con el P&L $ de cada CEDEAR y, en el tooltip, qué % representa ese resultado sobre el valor total de la cartera. Se decidió destruir y recrear el chart en cada render (no reusar instancia como hacen `comparison-chart`/`markowitz-chart`) porque el tooltip cierra sobre el total de la cartera, que cambia entre renders (cambio de ALyC, refresh de precios), y el branch de actualización in-place de `ChartManager.renderBarChart` no reconstruye los callbacks del tooltip — habría mostrado el % de impacto viejo
33. **Activos individuales en la Frontera Eficiente** (Análisis) — el gráfico de Markowitz (`renderMarkowitzChart`) solo mostraba carteras simuladas, la óptima y la actual; ahora suma un 4º dataset con cada CEDEAR como punto en su propio riesgo/retorno anualizado (tamaño del punto = peso en cartera), para ver de un vistazo cuáles activos aportan retorno sin arrastrar riesgo y cuáles son candidatos a recortar. El dato (`avgR`/`stdR` por activo) ya se calculaba dentro de `performMarkowitz` en `analysis-worker.js` para armar las carteras simuladas, solo no se exponía en el resultado — se agregó `assets: tickers.map(...)` al return del worker, sin cálculo nuevo
34. **Timing de tus Operaciones** (Análisis) — gráfico nuevo, no una variación de uno existente: línea de precio histórico por activo (selector de ticker) con las compras (▲ verde) y ventas (▼ roja) marcadas en la fecha exacta, para ver si las operaciones se hicieron en bajas o persiguiendo subas — algo que ningún gráfico de la página cruzaba hasta ahora (todo analizaba la foto actual de la cartera, nunca el timing de ejecución). Implementado con un mixed dataset de Chart.js (línea + puntos sin línea) sobre un eje de categorías con las fechas del historial de precios — se evitó a propósito agregar un adapter de fechas (`chartjs-adapter-date-fns`) nuevo; cada operación se alinea a la fecha disponible más cercana (ej. feriados sin vela), mismo criterio de alineación que ya usa `_calculateReturns`. De paso se evitó un bug real: la primera versión iba a filtrar operaciones por lista de tickers con `.in()`, pero el cliente Supabase de este proyecto (`supabase-minimal.js`) es una implementación propia reducida sin ese método — se resolvió trayendo todas las operaciones del ALyC (mismo patrón que ya usa el gráfico de Actividad) y filtrando por ticker en el cliente

### 💡 Evaluadas con el usuario, no elegidas (quedan como candidatas)
- Curva de concentración (Pareto) — peso % por ticker ordenado + línea de % acumulado, complementaría visualmente la alerta de concentración que hoy es solo texto (`getConcentrationAlert`)
- Evolución histórica real de la cartera (equity curve día a día) y/o underwater chart de drawdown en el tiempo — hoy Max Drawdown es solo un número; requiere reconstruir el valor histórico desde operaciones + precios, más laburo que las anteriores
- Redistribución sugerida (Actual/Sharpe/Michaud/HRP) como gráfico de barras agrupadas por ticker en vez de tabla — más clara para comparar cuánto coinciden las 3 estrategias, pero `ChartManager.renderStackedBarChart` fuerza `stacked: true` (sumaría en vez de comparar lado a lado); necesitaría una función nueva de barras agrupadas
